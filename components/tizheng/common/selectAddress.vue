<template>
  <div>
    <!--<div class="h_a">-->
      <!--<span><img src="../../../assets/img/cstz_7.png"/></span>-->
      ...
      <div class="h_b_b">
        <ul>
          ...
          <!--<li v-show="showCommunity" v-for="(item, index) in this.community.data" v-bind:key="'1-'+index">-->
            <!--<a class="b_b_b_0 b_b_b_1" target="" href="####" v-bind:title="item.properties.SQNAME"-->
    ...
    methods: {
      addressClick: function (index, type) {
        ...
        if(type === 0){
          ...
          bus.$emit('selectedAddress',this.address[index].name)
          ...
          bus.$emit('selectedAddress',this.address[index].name+'-'+this.address[this.index].data[index].properties.SQNAME)
...
<style lang="scss" scoped>
  ...
  .h_b_b ul li a:hover, .h_b_b ul li a:active, .h_b_b ul li a:focus { /*width:126px;*/
    height: 50px;
    display: block;

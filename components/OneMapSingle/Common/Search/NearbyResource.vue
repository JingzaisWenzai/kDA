...
<!--    <div class="resource_poi" v-else>
      <p class="resource_tit">{{page.line3}}</p>
      <div class="select_poi_content">
        <a v-for="(value,key,index) in ContentPage">
...
<style scoped>
  ...
  .resource_bg .resource_poi{height:62%;width: 90%;margin:0 auto;}
  .resource_bg div .resource_tit,
  .showNearBy div .resource_tit {color: #00d8ff;font-size:16px; padding:0;    position: relative;}
  .resource_bg div .resource_special {margin-top: 10px;margin-bottom: 10px;}
  .resource_bg div .resource_tit .back{cursor:pointer;display: inline-block;position:absolute;left:100%;margin-left: -30px; background: url('../../../../static/img/common/search/cross_dkg.png') no-repeat center;background-size: contain;    width: 18px;
    height: 18px;}
  .resource_bg .resource_poi .poi {overflow-x: hidden;overflow-y: auto;height: calc(100% - 35px);text-align:left;width:100%;}
  .poi .items{width:100%;clear:both;}
  .poi .items .poi_tit{font-size: 16px;color:#fff;padding:6px 0;width:100%;display: block;}
  .poi .items .poi_content{clear: both; width:100%;}
  .poi .items .poi_content span{ cursor: pointer;width: auto;display: block;float: left; color: #00d8ff;font-size: 14px;  padding:6px 8px;}
  ...
  .resource_bg .resource_btn a{ cursor: pointer;margin-top:10px;display: inline-block;color:#00d8ff;padding:2px 25px;width:auto;font-size: 14px; border: 1px solid #2f5ca9;-webkit-border-radius:5px;  -moz-border-radius:5px; border-radius:5px;}
  ...
  .showNearBy{    width: 100%;   height:22%;   margin-top: 4%;  }
  ...
  .select_poi_content{ width:100%;text-align: left;  margin:2% 0 0 0;overflow-x: hidden;overflow-y: auto;height:60%;}
  .select_poi_content a{ cursor: pointer; display: inline-block;vertical-align: middle;width:auto;height:auto;margin-right: 10px;color: #00d8ff;font-size: 14px; padding:2px 8px;}
  .select_poi_content a.on,
  .select_poi_content a:hover{color:#ffab27;}
  .select_poi_close{width:100%;text-align: right;}
  .select_poi_close a{display: inline-block; padding:2px 25px;width:auto;font-size: 14px; height:auto;cursor: pointer; background-color: #196db4;color:#fff;-webkit-border-radius:5px;  -moz-border-radius:5px; border-radius:5px;}
</style>
<script>

  import {getSearchResource} from '../../../../api/ecoapi';
  import TimeLine from './TimeLine.vue';
  import backUrl from '../../../../static/img/common/search/back_single.png';
  import resourch from '../../../../static/img/common/search/resource_nearby_single.png';
  import search from '../../../../static/img/common/search/search_nearby_single.png';
  ...
  export default{
    ...
    watch:{
      ...
      centerInfo : {
        handler:function(val,oldVal){
          console.log('3', val);
          this.center.wd = val.info.wd;
          this.center.jd = val.info.jd;
          this.center.name = val.info.name;
          this.center.layerid = val.layerid;
          this.center.nodeId = val.nodeId;
        },
        deep:true
      }*/
    },

...
<script>
  ...
  export default{
    ...
    mounted(){
        if(this.$store.state.historyChart.length !==0){
          this.doDraw();
        }
    },
    watch:{
    ...
    computed:{
        ...
        opt(){
            let that = this;
            let origin = that.dealData(that.theObj);
            let optt ={
              title:{
                show:false
              },
          ...
          if(oj.length !==0){
            ...
            for(let j=0;j<charts.length;j++){
              ...
              for(let i=0;i<brr.length;i++){
                if(crr.join('').indexOf(brr[i].time) === -1){
                  crr.push(brr[i].time);
                }
              }
            }
            // arr.time = crr.sort(function(val1,val2){
            //   if(val1<val2){
            //     return -1;
            //   }
            // });
            arr.time = crr.reverse()
            return arr;
          }
        },
        drawLine(){

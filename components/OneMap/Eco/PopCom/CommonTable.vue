<template>
  <div class="common_table">
    <table>
      <thead>
      <tr>
        <th v-if="theObj.length !== 0">序号</th><th v-for="(value, key, index) in theObj[0]">{{key}}</th>
      </tr>
      </thead>
      <tbody>
      <tr v-for="(item, index) in theObj" :key="index">
        <td>{{(Number(pages.pageNum) - 1) * Number(pages.pageSize) + index + 1}}</td><td v-for="(value,key,index) in item" >{{value}}</td>
      </tr>
      </tbody>
    </table>
  </div>
...
.common_table table{
  width: 100%;
  border:1px solid rgba(209,219,229,0.5);
...
.common_table table tbody tr:nth-child(odd){
...
<script>
  export default{
      ...
      data(){
          return {
              pages:{pageNum:1,pageSize:6,total:98}
             /* obj:[
                {
                   data:[

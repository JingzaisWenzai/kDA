  ...
  <div id="related" class="qiye_related">
    ...
    <div class="related-table" v-if="navContentStyle==='1'">
      ...
      <ul v-for="(a , b) in cardBox" :key ='b' >
        <p>{{a.name}}</p>
        <li v-for="(c , d) in a.data" :key='d'><span>{{c.name}}</span><span>:</span><span :title="c.value">{{c.value}}</span></li>
      </ul>
    </div>

    <div class="related-table only_table"  v-else-if="navContentStyle==='2'">
      <table>
        <thead>
        <tr>
          <th v-if="tableBox.thead.length !== 0">序号</th><th v-for="(e,f) in tableBox.thead" :key="f"> {{e}}</th><th v-if="tableBox.thead.length === 0">暂无数据</th>
        </tr>
        </thead>
        <tbody>
        <tr v-for ='(g,h) in tableBox.tbody' :key="h">
          <td>{{h + 1}}</td>
          <!--<td v-for="(i,o) in g" :key="o" :style="tableBox.thead[o].search('图片')!== -1||tableBox.thead[o].search('处置')!== -1?{cursor: 'pointer','font-size':'16px',textDecoration:'underline'}:{}" @click="tableBox.thead[o].search('图片')!== -1||tableBox.thead[o].search('处置')!== -1?showImag(i):showImag">{{tableBox.thead[o].search('图片')!== -1?'查看图片       （'+i.split('、').length+'张）':(tableBox.thead[o].search('处置')!== -1?'点击处置':i)}}</td>-->
          <td v-for="(i,o) in g" :key="o" :style="tableBox.thead[o].search('图片')!== -1||tableBox.thead[o].search('处置')!== -1?{cursor: 'pointer','font-size':'16px',textDecoration:'underline'}:{}" @click="tableBox.thead[o].search('图片')!== -1||tableBox.thead[o].search('处置')!== -1?showImag(i):showImag" v-html="tableBox.thead[o].search('图片')!== -1?'查看图片<br\>（'+(i===''?0:i.split('、').length)+'张）':(tableBox.thead[o].search('处置')!== -1?'详情':(i.replace('.0','')))"></td>
        </tr>
        </tbody>
      </table>
    </div>
    ...
    <div class="related-table"   v-else-if="navContentStyle === '3'">
      <ul>
        <li v-for="(value,key,index) in cardBoxContent" :key='index'><span>{{key}}</span><span>:</span><span :title="value">{{value}}</span></li>
      </ul>
    </div>
    <div class="related-table"   v-else-if="navContentStyle === '4'">
      <div style="color:#5fb6ff;margin-left: 15px;" class="table_title">{{tableData.title}}</div>
      <el-table
        :data="tableData.body"
    ...
    <a target="_blank" class="openWeiwen"></a>
  </div>
</template>
<script>
  import {getPopUp_ThreeS,getPopUp_Emergency,getSanxiao_placeScore} from '../../../../api/secapi';
  ...
  export default{
    ...
    mixins:[showImgAndCheckOutEvent],
      computed:{
        flag1(){
      ...
      methods:{
        async getData(nav,type){
          ...
          if(nav===0){
            ...
            console.log(nav)
            if(type==='1'){
              odata = await getSanxiao_placeScore({id:oid});
              data = odata;
              if(data) {
                this.tableData.title = data.name
                this.tableData.body = []
                for(let i = 0,j = data.data.length;i<j;i+=1){
                  for(let n = 0,m = data.data[i].data.length;n<m;n+=1){
                    this.tableData.body.push({
                      type:data.data[i].name,
                      name:data.data[i].data[n].name,
                      score:data.data[i].data[n].score,
                      value:data.data[i].data[n].value
                    })
                  }
                  this.tableData.body[this.tableData.body.length-data.data[i].data.length].index = data.data[i].data.length
                }
                this.navContentStyle = '4';
              }
            }
          }
          else if (nav === 3){
...
<style scoped>
  ...
  .qiye_related .navBar ul li{display: inline-block;vertical-align: middle;width: auto; height:100%; color: rgba(0,252,255,0.5);font-size: 22px;
  ...
  .qiye_related .related-table ul p{
    display: block;width: 100%;text-align: left;padding: 10px 0 10px 15px;color: #00fcff;font-size: 20px;
  }
  ...
  .only_table{overflow: auto!important;}
  /deep/ .el-table{

<template>
  <div id="related" class="yinhuan_related">
    ...
    <div class="navBar">
      <ul>
        <li :class=" contentBox  === index?'active':''" v-for="(item , index) in navBar" :key='index'
            @click="toChangeNav(index)">{{item}}
        </li>
      </ul>
    </div>
    ...
    <div class="related-table" v-else-if=" contentBox === 1">
      <ul>
        <li v-for="(value, key, index) in cardBox_obj" :key='index'>
          <span>{{key}}</span><span>:</span><span>{{value}}</span></li>
      </ul>
    </div>
    ...
    <div class="related-table" v-else-if=" contentBox === 3">
      ...
      <div class="the-table c_c_b_b">
        ...
        <table>
          ...
          <tbody v-for="(item,index) in buildScore.data" :key="index">
          <tr>
            <td :rowspan="item.data.length">
              <div class="c_c_b_b_a">{{item.name}}</div>
              <!--<div class="c_c_b_b_b">10%</div>-->
            </td>
            <td>{{item.data[0].name}}</td>
            <td>{{item.data[0].score}}</td>
            <td>{{item.data[0].value}}</td>
          </tr>
          <tr v-for="(item2,index2) in item.data" v-if="index2>0" :key="index2">
            <td>{{item2.name}}</td>
            <td>{{item2.score}}</td>
            <td>{{item2.value}}</td>
          </tr>
    ...
    <div class="related-table" v-else-if="contentBox === 5">   <!--建筑平平面纸-->
      ...
      <div class="listBox">
        <a v-for="(value,key,index) in listBox.list" :key="index"><span :title="key">{{key}}</span><span>:</span><span
          :title="value">{{value}}</span></a>
      </div>
    </div>
  </div>
</template>
<script>
  ...
  import {
    getPopUp_BuildingRisk_basic,
    getPopUp_BuildingRisk_duty,
    getPopUp_BuildingRisk_plain,
    getPopUp_BuildingRisk_risk,
    popup_BuildingRisk_hidden_danger,
    getPopUp_BuildingRisk_enterprise
  } from '../../../../api/secapi'
  ...
  export default {
    ...
    // mounted() {
    //   this.flag = this.$store.state.ShowYinHuan
    //   console.log(this.flag+'4444444444')
    // },
    watch: {
      flag: {
    ...
    methods: {
      ...
      async getData(id, nav) {
        ...
        if (nav === 0) { /*基本信息*/
          ...
          odata = await getPopUp_BuildingRisk_enterprise({
            id: id,
          ...
          odata = await  getPopUp_BuildingRisk_risk({
            id: id,
            pageNum: this.pageInfo.pageNum,
            pageSize: this.pageInfo.pageSize
          })
          ...
          odata = await  popup_BuildingRisk_hidden_danger({
            id: id,
            pageNum: this.pageInfo.pageNum,
            pageSize: this.pageInfo.pageSize
          })
          ...
          console.log('建筑平面')
          odata = await  getPopUp_BuildingRisk_plain({
            id: id,
            pageNum: this.pageInfo.pageNum,
            pageSize: this.pageInfo.pageSize
          })
...
<style scoped>
  ...
  ::-webkit-scrollbar-thumb {
    ...
    box-shadow: 0 0 0 5px rgba(25, 109, 180, 0.5) inset;
  }
  ...
  .yinhuan_related .navBar ul li:hover, .yinhuan_related .navBar ul li.active {
    color: #ffffff;
  ...
  .yinhuan_related .related-table ul li span {
    display: inline-block;
    vertical-align: middle; /*max-width: 50%;*/
    max-width: 70%;
    overflow: hidden;
    white-space: nowrap;
  ...
  .yinhuan_related .related-table ul li span:nth-child(2) {
    margin-left: 8px;
    margin-right: 10px;
  }

  .yinhuan_related .related-table ul li span:nth-child(3) {
    color: #a2d0ec;
  }
  ...
  .yinhuan_related .related-table .imgBox img {
    display: block;
    width: 90%;
    height: 100%;
    max-height: 100%;
  ...
  .yinhuan_related .related-table .listBox a {
    display: block;
    width: 95%;
    margin: 0 auto;
    text-align: left;
  ...
  .yinhuan_related .related-table .listBox a span {
    display: inline-block;
    vertical-align: middle;
    width: auto;
    color: #7af3ff;
  ...
  .yinhuan_related .related-table .listBox a span:nth-child(2) {
    margin: 0 8px 0 6px;
  }

  .yinhuan_related .related-table .listBox a span:nth-child(3) {
    color: #a2d0ec;
  }
</style>

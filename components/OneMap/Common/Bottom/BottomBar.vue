...
<script>
  ...
  export default{
    ...
    watch:{
      ...
      navFocusBtn(newVal,oldVal){
        if(newVal === '' || newVal === oldVal){
          /*   this.subBar 不变*/
        }else{
          //确认是生态环境一张图还是安全隐患一张图
          let goalID = this.oid === 'sec' ?  '.bottom_nav_type2 a' : '.bottom_nav span' ;
          this.subContent.data=[];
          let arr = this.navBar;
          let brr=[],flag;
        ...
        if(!this.clickedBtn.flag2.clicked){
          ...
          if(item.sub){
            ...
            if(oid === 'sec'){     /*安全隐患一张图*/
              ...
              this.$store.state.eco.dotType = item.dotType;
            }
          }
        }else{
...
<style scoped>
  ...
  .bottom_container .bottom_pop .middleware a,
  .bottom_container .three_block .three_block_cont a,
  .bottom_container .bottom_nav span,
  .bottom_container .bottom_nav_type2 a{
    cursor: pointer;
  }
  ...
  .bottom_container .bottom_pop span{font-size: 18px;color:#7df7ff;font-weight: 500;}
  ...
  .bottom_container .bottom_nav span{
    border-top:1px solid #008cff;border-bottom:1px solid #008cff; background-color: #001f35;display: inline-block;vertical-align: middle;font-size: 20px;color:#7df7ff;
    padding:15px 50px;
  }

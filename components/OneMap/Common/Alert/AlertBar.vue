  ...
  <div class="alert_container">
    ...
    <div class="bar_box" v-show="showSub==='open'">
      <table v-if="alert_value.thData.length!==0">
        ...
        <tbody>
        <tr v-for="(item,index) in alert_value.tbData" :key="index" class="serious" @click="loadSingelYuJing(alertInfo.tbData[index])"><td>{{index + 1}}</td><td v-for="(b,d) in item" :key="d" :title = 'b'>{{d === 2?b.replace('.0',''):b}}</td></tr>
        </tbody>
      </table>
      <div v-else>
        今日暂无预警
...
<style scoped>
  ...
  .alert_container .bar_content a{display: inline-block;vertical-align: middle;width: auto;margin-right: 20px;}
  ...
  .alert_container .bar_box{
    position: absolute;left:0;top:140%;width:800px;height:300px;
  ...
  .alert_container .bar_box table{ display: block;width: 95%; height:100%;margin:30px auto 0 auto;}
  .alert_container .bar_box table thead tr,.alert_container .bar_box table tbody tr{border-bottom: 1px solid #0f4e73;cursor: pointer;}
  ...
  .alert_container .bar_box table tbody tr td:nth-child(2),.alert_container .bar_box table thead tr th:nth-child(2){width:25%;}
  .alert_container .bar_box table tbody tr td:nth-child(3),.alert_container .bar_box table thead tr th:nth-child(3){width:40%;}
  .alert_container .bar_box table tbody tr td:nth-child(4),.alert_container .bar_box table thead tr th:nth-child(4){width:25%;}
  ...
  export default{
    ...
    mixins:[openFeatureLayerPopup],
      ...
      watch:{
        alertInfo : {
          handler:function(num,oldnum){
            console.log(num)
            let tHead = [],tbody = []
            for(let i = 2,j = num.thData.length;i<j;i+=1){
              tHead.push(num.thData[i])
            }
            for(let i = 0,j = num.tbData.length;i<j;i+=1){
              let tempTbody = []
              for(let n = 2,m = num.tbData[i].length;n<m;n+=1){
                tempTbody.push(num.tbData[i][n])
              }
              tbody.push(tempTbody)
            }
              this.alert_value.val = num.val;
              this.alert_value.thData = tHead;
              this.alert_value.tbData = [].concat(tbody);
          },
          deep:true
        },
        alert_value:{
      ...
      methods:{
        ...
        loadSingelYuJing(data){
          console.log(data)
          if(data.length>1){
            this.openFeatureLayerPopup(data[1], data[0])
          }
        }
      }
  }

 ...
 // name: 'VueDatepickerLocal',
  ...
  computed: {
    ...
    text () {
      ...
      let txt = this.dates.map(date => this.tf(date,this.format)).join(` ${this.rangeSeparator}`);
      this.$emit('childByValue', txt);
  ...
  methods: {
    ...
    tf (time, format) {
      ...
      const weeks = ['星期日','星期一','星期二','星期三','星期四','星期五','星期六'] // weeks
      const map = {
        YYYY: year,
        MM: dd(month + 1),
    ...
    /*console.log("sdfs");*/
      var tt = (format || 'W  YYYY-MM-DD').replace(/Y+|M+|W+|D+|H+|h+|m+|s+|S+/g, str => map[str]);
      return tt;
    },
    dc (e) {

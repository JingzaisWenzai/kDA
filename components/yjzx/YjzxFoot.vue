...
<script>
  ...
  export default {
    ...
    methods: {
      getAllMapLayer(){
        try {
          var node = document.getElementById('iframeGisMap').contentWindow.module.common.layerControl.mapLayerXml.getLayerNodeById('yujingzhongxin')
          let tempArray = []
          //先移除所有图层
          ...
          for(let i = 0,j = node.nodes.length;i<j;i+=1){
            if(node.nodes[i].nodes!==undefined){
              for(let n = 0,m = node.nodes[i].nodes.length;n<m;n+=1){
                tempArray.push(node.nodes[i].nodes[n].id)
                if(this.timeSelectIndex !== -1){
                  node.nodes[i].nodes[n].query = 'date=' + this.timeObj.data[this.timeSelectIndex]
                }
                document.getElementById('iframeGisMap').contentWindow.module.common.layerControl.mapLayerOperate.loadSingleLayerById(node.nodes[i].nodes[n].id)
              }
            }else {
              if(this.timeSelectIndex !== -1){
                node.nodes[i].nodes[n].query = 'date=' + this.timeObj.data[this.timeSelectIndex]
              }
              tempArray.push(node.nodes[i].id)
              document.getElementById('iframeGisMap').contentWindow.module.common.layerControl.mapLayerOperate.loadSingleLayerById(node.nodes[i].id)
            }
          }
      ...
      timeSelect(index, event) {//#ffab27
        ...
        bus.$emit('timeSelect',this.timeObj.data[index])

      },

      loadMapLay(){
        if(this.allLayer === true){
          //加载筛选过来的分类图层
          for (let i = 0, j = this.layerId.length; i < j; i += 1) {
            ...
            node.query = 'date=' + this.timeObj.data[this.timeSelectIndex]
      ...
      getMapData(code, layerId) {
        ...
        try {
          ...
          for (let i = 0, j = this.layerId.length; i < j; i += 1) {
            document.getElementById('iframeGisMap').contentWindow.module.common.layerControl.mapLayerOperate.removeAllLayer()
            // document.getElementById('iframeGisMap').contentWindow.module.common.layerControl.mapLayerOperate.removeSingleLayerById(this.layerId[i].id)
            var tempNode = document.getElementById('iframeGisMap').contentWindow.module.common.layerControl.mapLayerXml.getLayerNodeById(this.layerId[i].id)
            if(this.timeSelectIndex === -1){
              tempNode.query = 'date='
            }else {
              tempNode.query = 'date=' + this.timeObj.data[this.timeSelectIndex]
            }
            document.getElementById('iframeGisMap').contentWindow.module.common.layerControl.mapLayerOperate.loadSingleLayerByNode(tempNode)
          }
        }catch (e) {
          console.log(e.message)
          let $this = this
          setTimeout(function() {
            $this.getMapData(code, layerId)
          },2000)
        }
